...
export function EmergencyButton({ className }: { className?: string }) {
  return (
    <Link
      href="/request-blood"
      className={cn(
        "flex w-full items-center justify-center rounded-xl bg-emergency px-6 py-4 text-lg font-bold text-white shadow-lg transition-colors hover:bg-emergency-hover active:scale-[0.99]",
        className
      )}
    >
      NEED BLOOD NOW
