...
/**
 * Slim account strip for the donor AppShell.
 *
 * Brand mark is mobile-only: from `sm` up, DesktopNav already carries the
 * logo. Account controls (email + log out) stay here on every viewport so
 * hiding AuthNav on /donor does not remove the only way to sign out.
 *
 * Presentation only — `logoutAction` is the existing auth server action.
 */
export function DonorShellHeader({ email }: { email: string | null }) {
  return (
